...
#my_ip ='34.205.89.64'
#redis_client = redis.Redis(host=my_ip, port=6379)
folder_name = '/tmp/' #'/mnt/sgd/'
def download_imagenetdata():
    my_dir_set = set(os.listdir(folder_name))
    if 'train.rec' in my_dir_set:
        print("Test images already exist")
        return True
    file_name = ['train.idx','train.rec']
    mys3_clinet = boto3.resource('s3')
    BUCKET_NAME = 'imagenet-train-data'
    for my_file in file_name:
        print("downloading {}".format(my_file))
        dwonload_path = "{}{}".format(folder_name,my_file)#os.path.join(os.sep, './test', my_file)
        s3_path = my_file
        try:
            ...
            if e.response['Error']['Code'] == "404":
                print("Test images do not exist")
                exit(1)
    return True
# CLI
def parse_args():
    parser = argparse.ArgumentParser(description='Train a model for image classification.')
    parser.add_argument('--data-dir', type=str, default='{}imagenet'.format(folder_name),
                        help='training and validation pictures to use.')
    parser.add_argument('--rec-train', type=str, default='{}train.rec'.format(folder_name),
                        help='the training data')
    parser.add_argument('--rec-train-idx', type=str, default='{}train.idx'.format(folder_name),
                        help='the index of training data')
    parser.add_argument('--rec-val', type=str, default='{}val.rec'.format(folder_name),
                        help='the validation data')
    parser.add_argument('--rec-val-idx', type=str, default='{}val.idx'.format(folder_name),
    ...
    parser.add_argument('--model', type=str,  default='resnet18_v1',# resnet18_v1 #resnet50_v2
...
def sharding_dict(mydata, keys, shards, worker_id):
    ...
    #startup_nodes = [{"host": "3.222.188.240", "port": "6380"}, {"host": "3.222.188.240", "port": "6381"}, {"host": "3.222.188.240", "port": "6382"}]#, 
    #            #{"host": "3.222.188.240", "port": "7003"}, {"host": "3.222.188.240", "port": "7004"}, {"host": "3.222.188.240", "port": "7005"}]
    ...
    for i in range(shards):
        ...
        us = int(time.time()*1000)
        #print("start_uploading_worker_id_{}_shard_{}\t{}".format(worker_id, i, us))
        #print(redis_client.get('uplaod'))
        redis_client.set(file_name, pickle.dumps(data_dict))  
        ue = int(time.time()*1000)
        #print("end_uploading_worker_id_{}_shard_{}\t{}".format(worker_id, i, ue))
    e = int(time.time()*1000)
    #print("Total time {}".format(e-s))

def flat_sgd(shards, worker_id):
    with open("{}sgd_file.pkl".format(folder_name), "rb") as fp:
        data = pickle.load(fp)
    ...
    for ind, val in enumerate(data):
        ...
        if dimension > 3:
            ...
            keys.append(val)
    #shards = [100] #, 10, 50, 100]
    #for shard in shards:
    sharding_dict(flat, keys, shards, worker_id)


def main(batchsize, shards, worker_id):
    opt = parse_args()
    ...
    batch_size = batchsize #opt.batch_size
    classes = 1000
    num_training_samples = 1281167

    num_gpus = opt.num_gpus
    batch_size *= max(1, num_gpus)
    context = mx.cpu()#[mx.gpu(i) for i in range(num_gpus)] if num_gpus > 0 else [mx.cpu()]
    ...
    def get_data_rec(rec_train, rec_train_idx, rec_val, rec_val_idx, batch_size, num_workers):
        ...
        return train_data, batch_fn #, val_data,
    ...
    #if opt.use_rec:
    train_data, batch_fn = get_data_rec(opt.rec_train, opt.rec_train_idx,
                                                    opt.rec_val, opt.rec_val_idx,
                                                    batch_size, num_workers) #, val_data
    #else:
    #    train_data, batch_fn = get_data_loader(opt.data_dir, batch_size, num_workers) #, val_data
    ...
    def train(ctx):
        ...
        for epoch in range(opt.resume_epoch, opt.num_epochs):
            ...
            for i, batch in enumerate(train_data):
                ...
                #print("training iteration {} batch_size {}".format(i, np.shape(data[0])))
                #print("training iteration {} batch_size {}".format(i, len(data[0])))
                if opt.mixup:
                ...
                for val in net.collect_params():
                    my_dict[val] = net.collect_params()[val].data()
                    #print(my_dict[val])
                with open("{}sgd_file.pkl".format(folder_name), "wb") as fp:
                    pickle.dump(my_dict, fp)
                my_end = int(time.time()*1000)
                print("Pure_training_time\t{}".format(my_end-start))
                print("iteration\t{}\tticks\t{:.3f} sec".format(i, my_end-start))
                time.sleep(2)
                f_time = int(time.time()*1000)
                print("start_overall_upload_worker_id_{}\t{}".format(worker_id, f_time))
                flat_sgd(shards, worker_id)
                fe_time = int(time.time()*1000)
                print("finish_overall_upload_worker_id_{}\t{}".format(worker_id, fe_time))
...
def lambda_handler(event, context):
    ...
    print("Worker_execution_time\t{}".format(my_end-function_beign))
    return response
